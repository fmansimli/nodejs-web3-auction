...
import SecureLottie from "../assets/lotties/lock.json";
...
const Protected: React.FC<IProps> = (props) => {
  if (props.mount) {
    ...
    <div className="flex w-full items-center justify-center bg-white dark:bg-gray-700">
      <div className="container flex flex-1 flex-col items-center justify-center gap-5">
        <div className="w-full max-w-md">
          <Lottie animationData={SecureLottie} />
        </div>
        <div className="w-3/4 text-center text-lg font-bold text-black dark:text-white">
          <div className="text-md lg:text-lg">You need to sign in to visit that page.</div>
        </div>
        <Link
          to="/auth/signin"
